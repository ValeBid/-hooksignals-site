            ...
            <div className="grid gap-4">
              <div className="rounded-[24px] border border-cyan-300/20 bg-cyan-300/[0.06] p-5 md:p-6">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <p className="text-sm font-semibold text-cyan-300">Overall signal</p>
                  {mode && <span className="rounded-full border border-white/10 bg-black/25 px-3 py-1 text-xs text-white/45">{mode === "ai" ? "AI analysis" : "Beta fallback"}</span>}
                </div>
                <div className="mt-5 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    <div className="flex items-end gap-3">
                      <h2 className="text-7xl font-black tracking-tight text-cyan-300">{result.score}</h2>
              ...
              <div className="grid gap-4 lg:grid-cols-[1fr_0.9fr]">
                ...
                <div className="rounded-[24px] border border-white/10 bg-black/20 p-5">
                  <p className="mb-4 text-sm font-semibold text-white/55">AI reasoning</p>
                ...
                <div className="rounded-[24px] border border-white/10 bg-black/20 p-5">
                  ...
                  <div className="grid gap-3">
                    {result.feedback.map((item, index) => (
                      <div key={item} className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 leading-7 text-white/62"><span className="mr-2 font-bold text-violet-300">0{index + 1}</span>{item}</div>
                    ))}
                  </div>
                </div>
              )}
      ...
      {result && (
        <section className="mt-6 rounded-[28px] border border-white/10 bg-black/20 p-5 md:p-7">
          <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            ...
            <a href="/hook-improver" className="rounded-2xl bg-white px-5 py-3 text-sm font-bold text-black">Open Hook Improver</a>
          </div>
