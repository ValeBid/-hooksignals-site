...
const workflow = [
  ["01", "Analyze", "Find the weak point before publishing."],
  ["02", "Improve", "Rewrite vague hooks into sharper openings."],
  ["03", "Package", "Pair the hook with a clearer title and thumbnail."],
  ["04", "Publish", "Use retention signals before the video goes live."],
];

export default function ToolsPage() {
  return (
    <PremiumToolShell
      badge="Creator workflow dashboard"
      title="One premium system for stronger content decisions."
      description="Move from hook analysis to title, script, thumbnail and retention checks without the product feeling like a scattered tool list."
    ...
    >
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {tools.map((tool) => (
          <a
            key={tool.href}
            href={tool.href}
            className="group rounded-[24px] border border-white/10 bg-black/25 p-5 shadow-xl shadow-black/10 transition hover:border-emerald-300/40 hover:bg-emerald-300/[0.06] md:p-6"
          >
            <div className="mb-8 flex items-center justify-between">
              <span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs font-medium text-white/45">
                {tool.group}
              </span>
              <span className="flex h-10 w-10 items-center justify-center rounded-2xl border border-emerald-300/20 bg-emerald-400/10 text-sm font-black text-emerald-300 shadow-lg shadow-emerald-500/10 transition group-hover:scale-105">
                {tool.icon}
              </span>
            </div>
            ...
            <p className="mt-7 text-sm font-bold text-emerald-300">Open tool →</p>
          </a>
        ))}
      </div>
      ...
      <section className="mt-6 rounded-[24px] border border-emerald-300/20 bg-emerald-300/[0.06] p-6 md:p-8">
        ...
        <p className="mt-5 max-w-3xl leading-8 text-white/58">
          Creator performance is rarely decided by one element. A stronger hook improves retention, a clearer title improves clicks, and a sharper thumbnail improves the first impression.
        </p>

        <div className="mt-7 grid gap-3 md:grid-cols-4">
          {workflow.map(([step, title, desc]) => (
            <div key={step} className="rounded-2xl border border-white/10 bg-black/20 p-4">
              <p className="text-xs font-black text-emerald-300">{step}</p>
