    ...
    <PremiumToolShell badge="YouTube CTR tool" title="Thumbnail Text Checker" description="Check whether your thumbnail text is short, readable and clear enough to work inside the YouTube feed." primaryHref="/youtube-title-generator" primaryLabel="Generate Title" secondaryHref="/tools" secondaryLabel="All Tools">
      <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5 shadow-[0_24px_80px_rgba(0,0,0,.3)] backdrop-blur-xl md:p-7">
          ...
          {result && <div><p className="text-sm text-cyan-300">Thumbnail text score</p><div className="mt-3 flex items-end gap-3"><h2 className="text-7xl font-black text-cyan-300">{result.score}</h2><span className="mb-3 text-white/40">/100</span></div><p className="mt-5 rounded-2xl border border-white/10 bg-black/30 p-4 leading-7 text-white/65">{result.verdict}</p><div className="mt-5 grid gap-4 md:grid-cols-3"><Metric label="Length" value={result.lengthScore} /><Metric label="Clarity" value={result.clarityScore} /><Metric label="Mobile" value={result.mobileScore} /></div><div className="mt-5 space-y-3">{result.feedback.map((suggestion) => <div key={suggestion} className="rounded-2xl border border-white/10 bg-black/30 p-4 text-white/60">{suggestion}</div>)}</div><div className="mt-5 grid gap-3 md:grid-cols-3">{result.rewrites.map((rewrite) => <div key={rewrite} className="rounded-2xl border border-cyan-300/15 bg-cyan-300/[0.05] p-4 text-white/75"><p>“{rewrite}”</p><CopyButton text={rewrite} /></div>)}</div></div>}
        </div>
      </div>
