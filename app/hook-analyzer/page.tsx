...
const hookPrinciples = [
  ["Specific promise", "The viewer should understand the topic and expected payoff without decoding vague language."],
  ["Clear tension", "A strong hook creates a reason to keep watching: a mistake, contrast, result, risk or unanswered question."],
  ["Audience fit", "A hook for new creators should not sound like a hook for founders, fitness clients or finance beginners."],
  ["Packaging match", "The hook should support the same promise as the title and thumbnail. Mixed promises create early exits."],
];

const weakPatterns = [
  ["Too vague", "This changed everything", "Name what changed, who it affects and why the viewer should care."],
  ["No payoff", "You need to know this", "Add a concrete outcome, result, lesson or mistake."],
  ["Slow setup", "Today I want to talk about...", "Start with the viewer problem or the surprising result."],
];
...
export default function HookAnalyzerPage() {
  ...
  return <PremiumToolShell badge="Creator signal engine" title="AI Hook Analyzer for YouTube Shorts, TikTok and Reels" description="Score your opening line for clarity, curiosity, platform pacing, audience fit and retention risk before you publish." primaryHref="/checkout/pro" primaryLabel="Start Creator Pro" secondaryHref="/tools" secondaryLabel="All Tools"><TrustStrip /><div className="grid gap-5 xl:grid-cols-[0.72fr_1.28fr]"><div className="grid gap-5"><div className="rounded-[28px] border border-white/10 bg-black/30 p-5 shadow-[0_24px_80px_rgba(0,0,0,.35)] md:p-7"><div className="mb-5 grid gap-3 sm:grid-cols-3">{["Clarity", "Curiosity", "Retention"].map((item) => <div key={item} className="rounded-2xl border border-cyan-300/15 bg-cyan-300/[0.05] p-4"><p className="text-xs uppercase tracking-[0.16em] text-cyan-300">Signal</p><p className="mt-2 font-black text-white">{item}</p></div>)}</div><label htmlFor="hook-input" className="mb-3 block text-sm font-semibold text-white/62">Paste your hook</label><textarea id="hook-input" value={hook} onChange={(e) => setHook(e.target.value)} placeholder="Example: I uploaded 100 shorts in 30 days and only one changed everything." className="min-h-[180px] w-full resize-none rounded-[22px] border border-white/10 bg-[#050914] p-5 text-base leading-7 text-white outline-none transition placeholder:text-white/24 focus:border-cyan-300/40" /><div className="mt-3 flex items-center justify-between text-xs text-white/35"><span>{characterCount}/500 characters</span><span>{creditsRemaining === null ? "1 analysis = 5 credits" : `${creditsRemaining} credits left`}</span></div><div className="mt-5 grid gap-3 md:grid-cols-3"><div><label htmlFor="platform-select" className="mb-2 block text-xs font-bold uppercase tracking-[0.14em] text-white/35">Platform</label><select id="platform-select" value={platform} onChange={(e) => setPlatform(e.target.value)} className="w-full rounded-2xl border border-white/10 bg-[#050914] px-4 py-3 text-sm text-white outline-none focus:border-cyan-300/40"><option>YouTube Shorts</option><option>TikTok</option><option>Instagram Reels</option><option>YouTube Long-form</option></select><p className="mt-2 text-[11px] leading-5 text-white/32">Changes pacing, hook length and packaging advice.</p></div><div><label htmlFor="niche-input" className="mb-2 block text-xs font-bold uppercase tracking-[0.14em] text-white/35">Niche <span className="normal-case tracking-normal text-white/25">(optional)</span></label><input id="niche-input" value={niche} onChange={(e) => setNiche(e.target.value)} placeholder="Creator growth, fitness, AI..." className="w-full rounded-2xl border border-white/10 bg-[#050914] px-4 py-3 text-sm text-white outline-none placeholder:text-white/24 focus:border-cyan-300/40" /><p className="mt-2 text-[11px] leading-5 text-white/32">Makes titles, weaknesses and thumbnail angles specific.</p></div><div><label htmlFor="audience-input" className="mb-2 block text-xs font-bold uppercase tracking-[0.14em] text-white/35">Audience <span className="normal-case tracking-normal text-white/25">(optional)</span></label><input id="audience-input" value={audience} onChange={(e) => setAudience(e.target.value)} placeholder="New creators, founders..." className="w-full rounded-2xl border border-white/10 bg-[#050914] px-4 py-3 text-sm text-white outline-none placeholder:text-white/24 focus:border-cyan-300/40" /><p className="mt-2 text-[11px] leading-5 text-white/32">Improves viewer trigger and retention diagnosis.</p></div></div><div className="mt-4 grid gap-2 md:grid-cols-3">{contextTips.map((tip) => <div key={tip.title} className="rounded-2xl border border-white/10 bg-white/[0.025] p-3"><p className="text-xs font-black text-white/60">{tip.title}</p><p className="mt-1 text-[11px] leading-5 text-white/36">{tip.text}</p></div>)}</div>{error && <p className="mt-4 rounded-2xl border border-red-400/20 bg-red-400/10 p-3 text-sm text-red-100">{error}</p>}<button type="button" onClick={handleAnalyze} disabled={loading} className="mt-5 w-full rounded-2xl bg-gradient-to-r from-cyan-300 via-sky-400 to-violet-400 px-7 py-4 font-black text-black shadow-[0_20px_44px_rgba(34,211,238,.22)] transition hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-60">{loading ? "Analyzing hook signal..." : "Analyze Hook"}</button><div className="mt-5 rounded-[22px] border border-amber-300/20 bg-[linear-gradient(135deg,rgba(251,191,36,.11),rgba(34,211,238,.04))] p-4 text-sm leading-6 text-amber-50/85 shadow-[0_18px_46px_rgba(0,0,0,.18)]"><div className="flex gap-3"><span aria-hidden="true" className="mt-0.5 inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-amber-200/20 bg-amber-300/12 text-xs text-amber-100">✦</span><div><p className="font-black text-white">{mode === "ai" ? "Analysis saved" : mode === "rules" ? "Quick check" : "Private, credit-based analysis"}</p><p className="mt-1 text-white/58">{insightNotice}</p></div></div></div></div><LeftInsightRail hasResult={Boolean(result)} /></div><div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5 shadow-[0_24px_80px_rgba(0,0,0,.3)] backdrop-blur-xl md:p-7">{!loading && !result && <div className="min-h-[460px] rounded-[24px] border border-white/10 bg-black/20 p-6"><p className="text-sm text-cyan-300">Result preview</p><h2 className="mt-4 max-w-md text-3xl font-black tracking-tight">Your hook score will appear here.</h2><p className="mt-4 max-w-lg leading-7 text-white/50">HookSignals checks the opening promise, curiosity gap, audience fit and retention risk before publishing.</p></div>}{loading && <div className="min-h-[460px] rounded-[24px] border border-white/10 bg-black/20 p-6"><p className="text-sm text-cyan-300">Analyzing signal...</p><div className="mt-7 space-y-4"><div className="h-24 animate-pulse rounded-3xl bg-white/10" /><div className="h-3 w-full animate-pulse rounded-full bg-white/10" /><div className="h-3 w-5/6 animate-pulse rounded-full bg-white/10" /></div></div>}{result && <HookAnalysisResult result={result} mode={mode} />}</div></div><section className="mt-8 rounded-[34px] border border-white/10 bg-white/[0.03] p-6 md:p-8"><p className="text-sm font-black uppercase tracking-[0.14em] text-cyan-300">How the hook analyzer works</p><h2 className="mt-3 text-3xl font-black tracking-[-0.04em]">A useful hook score needs more than one generic rating.</h2><div className="mt-6 grid gap-4 md:grid-cols-2">{hookPrinciples.map(([title, text]) => <article key={title} className="rounded-[24px] border border-white/10 bg-black/24 p-5"><h3 className="text-xl font-black text-white">{title}</h3><p className="mt-3 text-sm leading-7 text-white/52">{text}</p></article>)}</div></section><section className="mt-6 rounded-[34px] border border-white/10 bg-black/24 p-6 md:p-8"><p className="text-sm font-black uppercase tracking-[0.14em] text-cyan-300">Hook score benchmarks</p><h2 className="mt-3 text-3xl font-black tracking-[-0.04em]">See what strong, average and weak hooks look like.</h2><div className="mt-6 grid gap-4 lg:grid-cols-3">{benchmarks.map((b) => <div key={b.hook} className="rounded-[26px] border border-white/10 bg-black/24 p-5"><div className="flex items-end gap-2"><p className="text-5xl font-black text-cyan-300">{b.score}</p><p className="mb-2 text-white/38">/100</p></div><p className="mt-2 text-sm font-black uppercase tracking-[0.14em] text-white/42">{b.label}</p><p className="mt-4 text-sm leading-6 text-white/76">“{b.hook}”</p><p className="mt-4 text-sm leading-6 text-white/45">{b.note}</p></div>)}</div></section><section className="mt-6 rounded-[34px] border border-white/10 bg-white/[0.03] p-6 md:p-8"><p className="text-sm font-black uppercase tracking-[0.14em] text-cyan-300">Common weak hook patterns</p><h2 className="mt-3 text-3xl font-black tracking-[-0.04em]">Fix the reason viewers leave early.</h2><div className="mt-6 grid gap-4 lg:grid-cols-3">{weakPatterns.map(([label, example, fix]) => <article key={label} className="rounded-[24px] border border-white/10 bg-black/24 p-5"><p className="text-sm font-black uppercase tracking-[0.14em] text-violet-200">{label}</p><p className="mt-4 text-sm leading-6 text-white/68">Weak: “{example}”</p><p className="mt-3 text-sm leading-6 text-white/48">Fix: {fix}</p></article>)}</div></section><section className="mt-6 grid gap-6 lg:grid-cols-[1fr_0.8fr]"><div className="rounded-[34px] border border-cyan-300/20 bg-[linear-gradient(135deg,rgba(34,211,238,.08),rgba(124,58,237,.08))] p-6 md:p-8"><p className="text-sm font-black uppercase tracking-[0.14em] text-cyan-300">HookSignals vs guessing</p><h2 className="mt-3 text-3xl font-black tracking-[-0.04em]">Stop publishing with a feeling. Use a pre-publish signal.</h2><div className="mt-6 grid gap-3 md:grid-cols-2">{["Retention analysis", "Audience triggers", "Title pairings", "Thumbnail angles", "Saved workspace history", "Credit-based usage"].map((item) => <p key={item} className="rounded-2xl border border-white/10 bg-black/24 p-4 text-sm text-white/66"><span aria-hidden="true" className="mr-2 text-emerald-300">✓</span>{item}</p>)}</div></div><div className="rounded-[34px] border border-white/10 bg-white/[0.035] p-6 md:p-8"><p className="text-sm font-black uppercase tracking-[0.14em] text-violet-200">Internal workflow</p><h2 className="mt-3 text-3xl font-black tracking-[-0.04em]">Turn one hook into a complete asset.</h2><div className="mt-6 grid gap-3">{[["/hook-improver", "Rewrite the hook with Hook Improver"], ["/youtube-title-generator", "Generate matching YouTube titles"], ["/thumbnail-text-checker", "Check thumbnail text clarity"], ["/shorts-script-generator", "Build a Shorts script opener"]].map(([href, label]) => <a key={href} href={href} className="rounded-2xl border border-white/10 bg-black/24 p-4 text-sm font-black text-white transition hover:border-cyan-300/30 hover:bg-cyan-300/[0.055]">{label} →</a>)}</div></div></section><section className="mt-6 rounded-[34px] border border-white/10 bg-black/24 p-6 md:p-8"><p className="text-sm font-black uppercase tracking-[0.14em] text-cyan-300">FAQ</p><h2 className="mt-3 text-3xl font-black tracking-[-0.04em]">Hook analyzer questions</h2><div className="mt-6 grid gap-4 md:grid-cols-2">{faqs.map((item) => <div key={item.q} className="rounded-[24px] border border-white/10 bg-white/[0.035] p-5"><h3 className="text-lg font-black text-white">{item.q}</h3><p className="mt-3 text-sm leading-6 text-white/52">{item.a}</p></div>)}</div></section><RelatedTools /></PremiumToolShell>;
}
